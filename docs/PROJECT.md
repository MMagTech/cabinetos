...
### THE ICON HAS TWO VARIANTS AND THE SECOND ONE IS EARNED — recorded 2026-09-22

**This was in nobody's notes and in neither repository.** Told by MMagTech
while choosing a startup screen, and found by grep to exist nowhere: the only
"afterburner" in either tree is a PCSX2 game-database comment about Sega. It
would have been lost.

| | |
|---|---|
| **The standard icon** | Cream cabinet, pink→amber marquee, and a **cyan→blue screen** (`#58E8F6`→`#2484D6`). Generated by `tools/make_icon.swift` in Cabinet. |
| **The Afterburner icon** | The same cabinet with a **warm screen** (`#FEE08F`→`#EB5736`) and a **lit power switch** glowing on the control panel. |

**AFTERBURNER MEANS THE PLATFORM RUNS MORE EMULATORS, AT FULL POWER.** It is not
a second style to pick from taste — it marks a tier. macOS has it because the
Mac runs the heavy systems. **MMagTech, 2026-09-22: CabinetOS has earned it**,
which is what PlayStation 2 on upstream PCSX2 and GameCube on Dolphin bought.

So **CabinetOS uses the Afterburner cabinet**, and anything that draws the icon
here — the boot splash, the startup screen, an About page — draws that one.

**AND `make_icon.swift` CANNOT PRODUCE IT.** The script emits only the cyan
version; the warm screen and the power switch are not in it. The Afterburner
artwork exists as PNGs nobody can regenerate, which is the same shape as the
Cabinet-side debt about a manifest that does not describe how a core is built.
**The values above were sampled out of `AppIconMac.appiconset/icon-1024.png`**,
because that file is currently the only definition there is.

**DRAW IT, DO NOT SHIP A PNG.** It is five rounded rectangles and four circles;
this frontend's renderer already draws all of those. Drawn, it is sharp at any
panel size, adds no asset to the image, and the marquee and screen can be
animated — which is what the splash below wants.
