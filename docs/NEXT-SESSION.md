...
**Next: System update (#70), ahead of File access and Storage.** MMagTech
took the recommendation, 2026-09-25, and the reason is a fact about the
image: **every automatic updater is switched off on purpose** (`uupd.timer`
disabled, `bootc-fetch-apply-updates.timer` masked, `strip-desktop.sh`), so
